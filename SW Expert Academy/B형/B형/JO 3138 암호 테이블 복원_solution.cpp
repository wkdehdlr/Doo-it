...
{
	for (int i = 0; i < idx; ++i) {
		if (hint[i][0][0] == hint[idx][0][0] && hint[i][0][1] == hint[idx][0][1])
		{
			return 1;
		}
	}
...
{
	N = n, M = n / 2;//N은 보드의 크기, M은 map의 크기
	idx = 0;
	empty = (M - 2)*(M - 2);
	encode(board);
