...
int update(int mode, int *y, int *m, int *d, int *date)
{
	if (mode == 0)return 1;
	if (mode >= 1 && mode <= 7) {
		*d += mode;
...
void set()
{
	...
	for (int i = 0; i < cnt; ++i) {
		//push
		if (date1 < 20180101) {
			if (update(mode, &yy, &mm, &dd, &date1))break;
			continue;
		}
...
void output()
{
	...
	if (date1 < 20171203)return;
	else if (date1 >= 20171203 && date1 <= 20171231)cnt = date1 + 30 - 1 - 20171231;
